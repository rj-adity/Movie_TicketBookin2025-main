...
const AdminSidebar = () => {
    ...
    // Fallback if not logged in or not admin

    // Always show 'Admin User' as name, but use real image if available
    const displayName = 'Admin User';
    const imageUrl = (user && user.imageUrl && isAdmin) ? user.imageUrl : assets.profile;
    ...
    return (
        <div className='h-[calc(100vh-64px)] md:flex flex-col items-center pt-8 max-w-13 md:max-w-60 w-full border-r border-gray-300/20 text-sm'>
            ...
            <p className='mt-2 text-base max-md:hiddden'>{displayName}</p>

...
const AddShows = () => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            // Fix: showsInput must have time as an array
            const showsInput = Object.entries(dateTimeSelection).map(([date, times]) => ({ date, time: times }));

            // Debug: log payload to verify structure
            console.log('Submitting show payload:', { movieId: selectedMovie, showsInput, showPrice: Number(showPrice) });
            ...
            toast.error('Error adding show. Please try again later.');
        }
        setAddingShow(false)
    }
    ...
    return nowPlayingMovies.length > 0 ? (
        <>
                    ...
                    {nowPlayingMovies.map((movie) => (
                        <div key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:translate-y-1 transition duration-300`} onClick={() => setSelectedMovie(movie.id)} >
                            <div className='relative rounded-lg overflow-hidden' >
                                ...
                                <div className='text-sm flex items-center justify-between p-2 black/70 w-full absolute bottom-0 left-0'>
            ...
            {/* Here I will Show You how to Show price Input*/}
            <div className='mt-8'>
